...
3. Run all reviewers in parallel with the same complete deliverable and
   ...
   Request read-only review, not implementation. Read the
   [reviewer result and recommendation contract](reviewer-contract.md) and
   include its result contract in every reviewer prompt.
...
7. Aggregate only after every seat reaches a terminal state. Surface every
   ...
   without a fix plan. Apply the combined recommendation rules in the
   [reviewer result and recommendation contract](reviewer-contract.md).
